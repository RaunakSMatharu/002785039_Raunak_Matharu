...
function editRow(r) {
  window.prompt("Go ahead! edit the details");
  selectRow();
}
...
function addRow() {
  ...
  student.innerHTML = `Student ${Math.ceil(rowC/ 2)}`;
  advisor.innerHTML = `Teacher ${Math.ceil(rowC/ 2)}`;
  ...
  try {
    setTimeout(() => {alert(`Next Student Added ${Math.ceil(rowC/ 2)}`)}, 100);
  } catch (error) {
